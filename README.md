...
Infrastructure as Code is considered a best practice when managing infrastructure in the cloud. Tools like Terraform are becoming increasingly popular due to their ease of use and multi cloud nature. However, adopting the Infrastructure as Code model can become hectic and unmanageable if not organized strategically. With automation simplicity is key to designing long lasting solutions.

In Azure DevOps, one way of simplifying Infrastructure as Code for Terraform configurations is by using pipeline templates to represent our infrastructure. Each value in the template parameters is used to customize our configuration which is then built dynamically during the build pipeline. We can go from the traditional repository structure of storing our Terraform configuration files in folders to leveraging pipeline template parameters to represent our infrastructure:

![TraditionalvsSimple](\Images\TraditionalvsSimple.PNG)

Structuring our source code repositories this way provides some of the following benefits which allow us to scale our solution much easier:
...
- We can now separate our Terraform code from our team deploying the configurations. All the Terraform modules and files are kept in another repository which we can limit access to just our Terraform team.
...
- Each deployment is going to be more consistent since we are using the same Terraform configurations in each deployment.

In this guide, we will use the Azure DevOps Demo Generator to import an Azure DevOps Project that has been set up to deploy Azure Container Instances with Terraform using pipeline templates as code. We will deploy some resources to our Azure subscription and review the innerworkings of this concept. 
...
First, we are going to import an Azure DevOps template project into our Azure DevOps organization. This will allow us to get started as quickly as possible. Navigate to the [Azure DevOps Demo Generator](https://azureDevOpsdemogenerator.azurewebsites.net/) website. 
...
Let's look at how this project is set up. On the left-hand side select **Repos** then choose **Files**. By selecting the drop down, we can see that there are two source code repositories or *repos* in our project. Select the **ACI** repo:

![SelectRepo](\Images\SelectRepo.PNG)

The **ACI** repo for this project contains the infrastructure code for the Azure Container Instances in our Azure environment with folders for Development and Production. In this project, instead of creating a repository for each application, or one for Dev and Prod, we are creating a repository for each cloud service or component.  This allows us to simplify our infrastructure and for our deployments to be as "cookie cutter" as possible which really shines with large environments. The environment size and business needs will really play a role on the most effective repo structure design. But, for this demonstration we are going to go this route to keep things simple for teaching the concept.
...
![terraformartifacts](\Images\terraformartifacts.PNG)
...
The Infrastructure as Code model can become difficult to manage at large scale, using pipeline templates instead of treating our Terraform config files like sheep prevents us from creating snowflake infrastructure and allows us to scale our infrastructure rapidly and in a stable way. For more information on Azure DevOps be sure to check out [Microsoft's documentation](https://docs.microsoft.com/en-us/azure/DevOps/?view=azure-DevOps). Also, if you're interested in learning more about Terraform take a look at their [website](https://www.terraform.io/intro/index.html) for more material. 
